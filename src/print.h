// Smart print debugger code
  ...
  void printScl(int line, char const *name, std::complex<float> x) {printf("%d float complex %s:    %g %+g i\n"  ,line,name,std::real(x),std::imag(x));}
  void printScl(int line, char const *name, std::complex<double> x) {printf("%d double complex %s:    %lg %+lg i\n" ,line,name,std::real(x),std::imag(x));}
  void printScl(int line, char const *name, std::complex<long double> x) {printf("%d long double complex %s:    %Lg %+Lg i\n" ,line,name,std::real(x),std::imag(x));}
  ...
  void printArr(int line, char const *name, std::complex<float> * x, int N) {
    ...
    if(N>0) {
      ...
      for(int i=0;i<N;i++) {
        if(i<PRINTLIMIT) printf("   %g %+g i,",std::real(x[i]),std::imag(x[i]));
        maxv = std::max(maxv,std::abs(x[i]));
        minv = std::min(minv,std::abs(x[i]));
      }
      printf("\b\nmax: %g, min: %g, length: %d\n",maxv,minv,N);
    }
  }
  void printArr(int line, char const *name, std::complex<double> * x, int N) {
    ...
    if(N>0) {
      ...
      for(int i=0;i<N;i++) {
        if(i<PRINTLIMIT) printf("   %lg %+lg i,",std::real(x[i]),std::imag(x[i]));
        maxv = std::max(maxv,std::abs(x[i]));
        minv = std::min(minv,std::abs(x[i]));
      }
      printf("\b\nmax: %lg, min: %lg, length: %d\n",maxv,minv,N);
  ...
  void printArr(int line, char const *name, std::complex<long double> * x, int N) {
    ...
    if(N>0) {
      long double maxv = std::abs(x[0]);
      long double minv = std::abs(x[0]);
      for(int i=0;i<N;i++) {
        if(i<PRINTLIMIT) printf("   %Lg %+Lg i,",std::real(x[i]),std::imag(x[i]));
        maxv = std::max(maxv,std::abs(x[i]));
        minv = std::min(minv,std::abs(x[i]));
