...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, mxArray const *prhs[]) {
	float complex *E_in = mxGetData(prhs[0]); // Input E field
    long nx = mxGetM(prhs[0]);
    long ny = mxGetN(prhs[0]);
    long nz = *mxGetDoubles(mxGetField(prhs[1],0,"nz")); // Number of steps to perform
	float complex *multiplier = mxGetData(mxGetField(prhs[1],0,"multiplier")); // Array of multiplier values to apply to the E field after each step, which includes (1) the phase imparted by the refractive index differences, (2) the absorber outside the fiber and (3) the effects of fibre bending, if present
    float complex ax = *(float complex *)mxGetData(mxGetField(prhs[1],0,"ax"));
    float complex ay = *(float complex *)mxGetData(mxGetField(prhs[1],0,"ay"));
	
    float complex *E_out = mxGetData(plhs[0] = mxCreateNumericArray(2,(mwSize[]){nx, ny},mxSINGLE_CLASS,mxCOMPLEX)); // Output E field

	float complex *E_temp = malloc(nx*ny*sizeof(float complex)); // Temporary E matrix
    
    float complex **E_src = &E_in; // Pointer to the pointer to whichever E matrix is to be read from
    float complex **E_tgt; // Pointer to the pointer to whichever E matrix is to be written to
