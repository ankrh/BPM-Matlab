...
Lx_main = 40e-6;                                                     % [m] x side length of main area
Ly_main = 40e-6;                                                    % [m] y side length of main area
Lz = 0.003;                                                         % [m] z propagation distance
targetzstepsize = 1e-6;                                          % [m] z step size to aim for

Nx_main = 60;                                                        % x resolution of main area
Ny_main = 60;                                                             % y resolution of main area

lambda = 1e-6;                                                % [m] Wavelength 
w_0 = 1.3*5.275e-6;                                                       % [m] Initial waist plane 1/e^2 radius of the gaussian beam

updates = 100;          % Number of times to update plot. If set higher than nz (such as Inf), script will simply update every step.
...
gamma = 3e16; % [1/m^3] "Absorption coefficient" per unit length distance out from edge of main area, squared, used for absorbertype 4
...
contour(X,Y,n_mat,(n_cladding+eps(n_cladding))*[1 1],'color','r','linestyle','--');
subplot(2,1,2);
hold on;
...
multiplier = exp(-1i*dz*k_0*(n_mat.^2 - n_0^2)/n_0).*absorber;
d = zeros(nx,ny);
b = zeros(max(nx,ny),1);
for zidx = 1:nz
