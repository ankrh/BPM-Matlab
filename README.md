...
#### Segments
BPM-Matlab will add a new segment to the simulation that uses as an input E-field the output of the previous segment with each call to the solver, see towards the end of this file. As such, you can stack segments one after the other by only re-defining the parameters that change. Check Example2.m for an example.
...
- `P.saveVideo`  
Set this to `true` to save the field intensity and phase profiles at different transverse planes to a video. Default is `false`.
- `P.videoName`  
Set the name of the video to be saved.
- `P.finalizeVideo`  
This controls whether the video should be finalised after the current segment. Set this to `true` if the current segment is the last segment in your simulation, and to `false` otherwise.
...
BPM-Matlab includes a mode solver that can calculate the supported modes of the waveguide and decompose the simulated E-field into these modes. Check Example9.m on how to do so. The modes are *not* used for the propagation simulation itself.
Tu use the mode solver, find the modes supported by the waveguide (after you set all the other parameters of P, especially P.shapes) by using
`P = findModes(P,nModes,sortByLoss,plotModes);` with

- `nModes`  
number of modes to find
- `plotModes`  
set to `true` if the solver should plot the modes during the simulation
- `sortByLoss`
...
This will set `P.modes`, a struct array of containing the supported modes. You should then set `P.calcModeOverlaps` to `true` to calculate mode overlap integrals of propagating field with respect to the different modes that were set in the `P.modes` struct array.
